...
import Helpers.grading
import imp
import optparse
import os
import re
import sys
import Helpers.projectParams
import random
random.seed(0)
try:
...
def readCommand(argv):
    ...
    parser.add_option('--test-directory',
                      dest='testRoot',
                      default='Helpers/test_cases',
                      help='Root test directory which contains subdirectories corresponding to each question')
    parser.add_option('--student-code',
                      dest='studentCode',
                      default=Helpers.projectParams.STUDENT_CODE_DEFAULT,
    ...
    parser.add_option('--test-case-code',
                      dest='testCaseCode',
                      default=Helpers.projectParams.PROJECT_TEST_CLASSES,
...
def runTest(testName, moduleDict, printTestCase=False, display=None):
    ...
    testDict = Helpers.testParser.TestParser(testName + ".test").parse()
    solutionDict = Helpers.testParser.TestParser(testName + ".solution").parse()
    test_out_file = os.path.join('%s.test_output' % testName)
    testDict['test_out_file'] = test_out_file
    testClass = getattr(Helpers.projectTestClasses, testDict['class'])

    questionClass = getattr(Helpers.testClasses, 'Question')
    ...
    grades = Helpers.grading.Grades(Helpers.projectParams.PROJECT_NAME, [(None, 0)])
...
def evaluate(generateSolutions, testRoot, moduleDict, exceptionMap=ERROR_HINT_MAP,
    ...
    questions = []
    questionDicts = {}
    test_subdirs = getTestSubdirs(Helpers.testParser, testRoot, questionToGrade)
    for q in test_subdirs:
        ...
        questionDict = Helpers.testParser.TestParser(
            os.path.join(subdir_path, 'CONFIG')).parse()
        questionClass = getattr(Helpers.testClasses, questionDict['class'])
        ...
        for t in sorted(tests):
            ...
            testDict = Helpers.testParser.TestParser(test_file).parse()
            ...
            def makefun(testCase, solution_file):
                if generateSolutions:
                    ...
                    testDict = Helpers.testParser.TestParser(test_file).parse()
                    solutionDict = Helpers.testParser.TestParser(solution_file).parse()
                    if printTestCase:
    ...
    grades = Helpers.grading.Grades(Helpers.projectParams.PROJECT_NAME, questions,
    ...
    grades.grade(sys.modules[__name__], bonusPic=Helpers.projectParams.BONUS_PIC)
...
def getDisplay(graphicsByDefault, options=None):
    ...
    if graphics:
        try:
            ...
            return Helpers.graphicsDisplay.PacmanGraphics(1, frameTime=.05)
        except ImportError:
            pass
    import Helpers.textDisplay
    return Helpers.textDisplay.NullGraphics()
...
if __name__ == '__main__':
    ...
    moduleDict = {}
    for cp in codePaths:
        moduleName = re.match('.*?([^/]*)\.py', cp).group(0)[:-3]
        moduleDict[moduleName] = loadModuleFile(
            moduleName, os.path.join(options.codeRoot, cp))
    moduleName = re.match('.*?([^/]*)\.py', options.testCaseCode).group(0)[:-3]
