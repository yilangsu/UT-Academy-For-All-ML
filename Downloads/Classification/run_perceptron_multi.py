...
import Helpers.mostFrequent
import Helpers.naiveBayes
import Exercises.perceptron_multiclass
import Exercises.perceptron_pacman
import Helpers.mira
import Helpers.samples
import sys
import Helpers.util
import random
...
def basicFeatureExtractorPacman(state):
    ...
    for action in state.getLegalActions():
        ...
        featureCounter = Helpers.util.Counter()
...
def enhancedFeatureExtractorPacman(state):
    ...
    for action in state.getLegalActions():
        features[action] = Helpers.util.Counter(features[action], **enhancedPacmanFeatures(state, action))
    return features, state.getLegalActions()
...
def enhancedPacmanFeatures(state, action):
    ...
    nearest_ghosts = sorted([Helpers.util.manhattanDistance(pac_pos, i) for i in ghosts])

    features["nearest_ghost"] = nearest_ghosts[0] * 1.0

    for i in range(min(len(nearest_ghosts), 1)):
        features[f"ghost-{i}"] = 5 / (0.1 + nearest_ghosts[i])

    nearest_caps = sorted([Helpers.util.manhattanDistance(pac_pos, i) for i in capsules])

    for i in range(min(len(nearest_caps), 1)):
        features[f"capsule-{i}"] = 15 / (1 + nearest_caps[i])

    nearest_food = sorted([Helpers.util.manhattanDistance(pac_pos, i) for i in food])

    for i, weight in zip(range(min(len(nearest_food), 5)), [1.3, 0.8] + [0.9] * 3):
        features[f"food-{i}"] = weight * nearest_food[i]
...
class ImagePrinter:
    ...
    def printImage(self, pixels):
        ...
        """
        image = Helpers.samples.Datum(None,self.width,self.height)
        for pix in pixels:
            try:
...
def readCommand( argv ):
    ...
    if(options.classifier == "mostFrequent"):
        classifier = Helpers.mostFrequent.MostFrequentClassifier(legalLabels)
    elif(options.classifier == "naiveBayes" or options.classifier == "nb"):
        classifier = Helpers.naiveBayes.NaiveBayesClassifier(legalLabels)
        ...
        if options.data != 'pacman':
            classifier = Exercises.perceptron_multiclass.PerceptronClassifier(legalLabels,options.iterations)
        else:
            classifier = Exercises.perceptron_pacman.PerceptronClassifierPacman(legalLabels,options.iterations)
    elif(options.classifier == "mira"):
        if options.data != 'pacman':
            classifier = Exercises.mira.MiraClassifier(legalLabels, options.iterations)
        if (options.autotune):
...
MAP_AGENT_TO_PATH_OF_SAVED_GAMES = {
    'FoodAgent': ('Helpers/pacmandata/food_training.pkl','Helpers/pacmandata/food_validation.pkl','Helpers/pacmandata/food_test.pkl' ),
    'StopAgent': ('Helpers/pacmandata/stop_training.pkl','Helpers/pacmandata/stop_validation.pkl','Helpers/pacmandata/stop_test.pkl' ),
    'SuicideAgent': ('Helpers/pacmandata/suicide_training.pkl','Helpers/pacmandata/suicide_validation.pkl','Helpers/pacmandata/suicide_test.pkl' ),
    'GoodReflexAgent': ('Helpers/pacmandata/good_reflex_training.pkl','Helpers/pacmandata/good_reflex_validation.pkl','Helpers/pacmandata/good_reflex_test.pkl' ),
    'ContestAgent': ('Helpers/pacmandata/contest_training.pkl','Helpers/pacmandata/contest_validation.pkl', 'Helpers/pacmandata/contest_test.pkl' )
}
# Main harness code
...
def runClassifier(args, options):
    ...
    if(options.data=="pacman"):
        ...
        rawTrainingData, trainingLabels = Helpers.samples.loadPacmanData(trainingData, numTraining)
        rawValidationData, validationLabels = Helpers.samples.loadPacmanData(validationData, numTest)
        rawTestData, testLabels = Helpers.samples.loadPacmanData(testData, numTest)
    else:
        rawTrainingData = Helpers.samples.loadDataFile("Helpers/digitdata/trainingimages", numTraining,DIGIT_DATUM_WIDTH,DIGIT_DATUM_HEIGHT)
        trainingLabels = Helpers.samples.loadLabelsFile("Helpers/digitdata/traininglabels", numTraining)
        rawValidationData = Helpers.samples.loadDataFile("Helpers/digitdata/validationimages", numTest,DIGIT_DATUM_WIDTH,DIGIT_DATUM_HEIGHT)
        validationLabels = Helpers.samples.loadLabelsFile("Helpers/digitdata/validationlabels", numTest)
        rawTestData = Helpers.samples.loadDataFile("Helpers/digitdata/testimages", numTest,DIGIT_DATUM_WIDTH,DIGIT_DATUM_HEIGHT)
        testLabels = Helpers.samples.loadLabelsFile("Helpers/digitdata/testlabels", numTest)
    ...
    #     guesses.append(classifier.classify(validationData[i]))
    ...
    #     guesses.append(classifier.classify(testData[i]))
